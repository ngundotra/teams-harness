...
- Ungate already applied (`node scripts/ungate-playground.mjs` injects `scripts/pg-restyle.js`).
...
- Content-loop pass is a later grok bubble (`original text:` / queued follow-ups). Ack is a 👀 **chip** on the target plus `Working on it...`. A visible `👀`/`⭐` prefix-copy bubble is a fail.
- React-loop pass (3, 6, 10) is a ⭐ **chip** on the target message in Playground chrome. `/debug/state` `setReaction` is supporting evidence, not the receipt.
...
3. [DM intra-turn star](./03-dm-intra-turn-star.md) — `star this` while Working on it; ⭐ chip on the target (prefix-copy bubble is a fail).
...
10. [In-thread star](./10-in-thread-star.md) — new root post, `star this` in thread while Working on it; ⭐ chip on the target.
